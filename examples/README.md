...
| `observy_vita.nim` | trace + metric + log | PlayStation Vita headless build, plaintext OTLP/protobuf to `10.0.0.106:4318`; build with `scripts/build_vita.sh examples/observy_vita.nim observy_vita`; read `ux0:data/observy_vita_result.txt` after launch (`PASS` means traces, metrics, and logs all returned HTTP 200); set `NIMFLAGS_VITA='-d:ObservyVitaUtcOffsetSec=<seconds>'` if the Vita clock is local time rather than UTC |
